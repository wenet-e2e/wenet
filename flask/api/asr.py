...
def Record():
    if request.method == 'POST':
        request_id = 'request-' + str(uuid.uuid1())
        ret = {'ret_code': 1, 'audio_duration': 0, 'sn': ''}
        ret['ret_msg'] = 'recognize successfully!'
        info_json = {'interface': 'ASR', 'method': request.method}
        info = json.dumps(info_json)
        print(info)
        duration = 0
        try:
            ...
            if len(data) > 0:
                ret['sn'] = request_id
                decode_start = time.time()
                pcm, duration = decode_strict_wav(data, SAMPLE_RATE_JS)
                status, result = wenet_handler.recognize(pcm, SAMPLE_RATE_JS)
                if not status:
                    raise Exception(result)
                ret['result'] = result
                decode_elasped = time.time() - decode_start
        ...
        info = 'access ip:{}, url:{}'.format(request.remote_addr, request.url)
        print(info)
        record_js = url_for('static', filename='js/recorder.wav.min.js')
        main_js = url_for('static', filename='js/record.js')
        content = render_template('record.html', main=main_js, record=record_js)
