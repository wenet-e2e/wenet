...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    utils/data/modify_speaker_info.sh --seconds-per-spk-max 180 $wave_data/${dset}.orig $wave_data/${dset}
    done
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    if [ ! -f $wave_data/$train_set/segments ]; then
        ...
        for x in $train_dev $train_set $recog_set; do
            ...
            python tools/segment.py --segments $wave_data/$x/segments \
                --input $wave_data/$x/wav.scp.ori \
                 --output $wave_data/$x/wav.scp
        done
    fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # change format.data.sh here to support sph
    for x in $train_dev $train_set $recog_set; do
        tools/format_data.sh --nj ${nj} --feat-type "opus" --feat $wave_data/$x/wav.scp \
        --bpecode ${bpemodel}.model $wave_data/$x ${dict} > $wave_data/$x/format.data
    done
fi
