...
# 4. conf/train_conformer_torchaudio_wav_distort.yaml: 
train_config=conf/train_conformer_torchaudio.yaml
#train_config=conf/train_conformer_torchaudio_wav_distort.yaml
checkpoint=
cmvn=true
#eheckpoint=exp/torchaudio_wav_distort_refine/1.pt
#dir=exp/torchaudio_wav_distort
dir=exp/torchaudio_2gpu_layernorm_cmvn
#dir=exp/torchaudio_wav_distort_0.2
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    dist_backend="nccl"
    cmvn_opts=
    $cmvn && cmvn_opts="--cmvn ./global_cmvn"
    ...
    for ((i = 0; i < $num_gpus; ++i)); do
    {
        ...
        python wenet/bin/train_torchaudio.py --gpu $gpu_id \
            --config $train_config \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for mode in ctc_greedy_search ctc_prefix_beam_search attention attention_rescoring; do
    {
        ...
        mkdir -p $test_dir
        python wenet/bin/recognize_torchaudio.py --gpu 1 \
            --mode $mode \
