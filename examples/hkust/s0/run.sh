...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    # Data preparation
    local/hkust_data_prep.sh /mnt/cfs/database/hkust/LDC2005S15/ /mnt/cfs/database/hkust/LDC2005T32/ || exit 1;
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for mode in ${decode_modes}; do
    {
      ...
      if [ $en_modeling_unit == "bpe" ]; then
         tools/spm_decode --model=${bpecode} --input_format=piece < $test_dir/text_${en_modeling_unit} | sed -e "s/▁/ /g" > $test_dir/text
      else
