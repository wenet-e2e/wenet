...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in train dev; do
        ...
        paste -d " " <(cut -f 1 -d" " data_${en_modeling_unit}/${x}/text.org) <(cut -f 2- -d" " data_${en_modeling_unit}/${x}/text.org \
    ...
    for x in ${test_sets}; do
        cp data_${en_modeling_unit}/test_${x}/text data_${en_modeling_unit}/test_${x}/text.org
        paste -d " " <(cut -f 1 -d" " data_${en_modeling_unit}/test_${x}/text.org) <(cut -f 2- -d" " data_${en_modeling_unit}/test_${x}/text.org \
            | tr 'a-z' 'A-Z' | sed 's/\([A-Z]\) \([A-Z]\)/\1▁\2/g' | sed 's/\([A-Z]\) \([A-Z]\)/\1▁\2/g' | tr -d " ") \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    tools/text2token.py -s 1 -n 1 -m ${bpecode} data_${en_modeling_unit}/${train_set}/text ${trans_type_ops} | cut -f 2- -d" " | tr " " "\n" \
    ...
    for x in ${dev_set} ${train_set} ${feat_test_sets}; do
        if [ $data_type == "shard" ]; then
            tools/make_shard_list.py --num_utts_per_shard $num_utts_per_shard \
                --num_threads 16 data_${en_modeling_unit}/$x/wav.scp data_${en_modeling_unit}/$x/text \
                $(realpath data/$x/shards) data_${en_modeling_unit}/$x/data.list
        else
            tools/make_raw_list.py data_${en_modeling_unit}/$x/wav.scp data_${en_modeling_unit}/$x/text \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for mode in ${decode_modes}; do
    {
        for x in ${test_sets}; do
        {
            ...
            #fi
            cat data_${en_modeling_unit}/test_${x}/text | sed -e "s/▁/ /g" > data_${en_modeling_unit}/test_${x}/text.tmp
            python tools/compute-wer.py --char=1 --v=1 \
                data_${en_modeling_unit}/test_${x}/text.tmp $test_dir/text > $test_dir/wer
