    ...
    def forward(
        ...
        Args:
            ...
            mask (torch.Tensor): Mask tensor (#batch, ...,  1, time2) or
                ...
                Wenet.
            cache (torch.Tensor): Cache tensor (B, ..., head, cache_t, d_k * 2),
        ...
        Returns:
            ...
            torch.Tensor: Cache tensor (batch, head, cache_t + time1, d_k * 2)
                where `cache_t == chunk_size * num_decoding_left_chunks`
                and `head * d_k == size`
        ...
        if not self.use_sdpa:
            ...
            output = output.transpose(-2, -3).contiguous().view(
                output.size()[:-2] +
                torch.Size([self.h * self.d_k]))  # (batch, time1, d_model)
...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def forward(
        ...
        """
        q, k, v = self.forward_qkv(query, key, value)
        q = q.transpose(-3, -2)  # (batch, ..., time1, head, d_k)
        ...
        pos_shape = pos_emb.size()[:-1] + torch.Size([self.h, self.d_k])
        p = self.linear_pos(pos_emb).view(pos_shape)
        p = p.transpose(-2, -3)  # (batch, head, time1, d_k)
        ...
        if not self.use_sdpa:
            ...
            output = output.transpose(-2, -3).contiguous().view(
                output_shape)  # (batch, ..., time1, d_model)
            return self.linear_out(output), new_cache
