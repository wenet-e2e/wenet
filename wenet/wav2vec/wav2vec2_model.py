...
from wenet.wav2vec.wav2vec2_encoder import W2vConformerEncoder
from wenet.wav2vec.w2v_loss import W2vLoss
...
class Wav2vec2Model(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pretrain=wav2vec_conf.get('pretrain',False)

        self.w2v_ext_loss=wav2vec_conf.get('w2v_ext_loss',True)
        if self.w2v_ext_loss:
            self.w2v_loss_weights=wav2vec_conf.get('w2v_loss_weights',[0.1,10])
        else:
            self.w2v_loss_weights=None
        if self.pretrain:
            self.w2v_loss=W2vLoss(loss_weights=self.w2v_loss_weights)

        self.encoder_grad_mult=wav2vec_conf.get('encoder_grad_mult',1.0)
        self.freeze_finetune_updates=wav2vec_conf.get('freeze_finetune_updates',0)
    
        self.num_updates=0

        self.log_interval=wav2vec_conf.get('log_interval',100)
        self.accum_grad=wav2vec_conf.get('accum_grad',4)
    ...
    def forward(
        ...
        if self.pretrain:
            loss,sample_size,logging_output=self.w2v_loss(self.encoder,ext_res)
            loss_att=None
            loss_ctc=None
            if self.num_updates*self.accum_grad % 100 ==0:
...
def init_wav2vec2_model(configs):
    ...
    if encoder_type == 'conformer':
        encoder = W2vConformerEncoder(wav2vec_conf,
                                   input_dim,
