...
import wenet.tts.vits.commons as commons
import wenet.tts.vits.modules as modules
import wenet.tts.vits.attentions as attentions
import wenet.tts.vits.monotonic_align as monotonic_align
...
class VitsModel(nn.Module):
    ...
    def __init__(self, n_vocab, spec_channels, **kwargs):
        super().__init__()
        ...
        self.segment_size = kwargs.get('segment_size', 8192)
        self.c_mel = kwargs.get('c_mel', 45)
        self.c_kl = kwargs.get('c_kl', 1.0)
        self.d_interval = kwargs.get('d_interval', 2)
        self.g = SynthesizerTrn(n_vocab, spec_channels,
    ...
    def forward(self, batch: dict, device: torch.device):
        ...
        (y_hat, l_length, attn, ids_slice, x_mask, z_mask,
         (z, z_p, m_p, logs_p, m_q, logs_q)) = self.g(x, x_lengths, spec,
                                                      spec_lengths, sid)
        ...
        if optimizer_idx == 0:
            ...
            losses = {'loss': loss_disc_all, 'loss_disc': loss_disc_all}
        else:
            # Generator loss
    ...
    def infer(self, text: torch.Tensor):
        assert text.dim() == 1
        device = text.device
        x_length = torch.tensor([text.size(0)],
                                device=device,
                                dtype=torch.long)
        x = text.unsqueeze(0)
        sid = torch.zeros(1, device=device, dtype=torch.long)
        audio, *_ = self.g.infer(x, x_length, sid=sid)
        return audio
