...
prefix = sys.argv[1]
print("add prefix \"{}\" for each word, i.e., from \'hello\" to \"{}hello\"".format(prefix, prefix))
...
with open(sys.argv[2], 'r', encoding='utf8') as fin, \
    ...
    while (line) :
        ...
        try :
            prob = float(prob)
            words = line.split('\t')[1]
            back_prob = line.split('\t')[2] if (len(line.split('\t')) > 2) else None
            words = " ".join([prefix + word if (word[0] != '<') else word
                                for word in words.split()])
            fout.write('{}\t'.format(prob))
            fout.write('{}'.format(words))
