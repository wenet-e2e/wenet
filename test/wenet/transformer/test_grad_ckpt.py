...
@pytest.mark.parametrize(
    "module", [TransformerEncoder, ConformerEncoder,
...
def test_encoder_gradient_checkpointing(module):
    ...
    if module in [TransformerEncoder, ConformerEncoder]:
        ...
        logits_grad_ckpt = model_grad_ckpt(memory, memory_mask, tgt, tgt_lens, r_tgt)[0]
    else:
    ...
    for (name1, param1), (name2, param2) in zip(
            model.named_parameters(), model_grad_ckpt.named_parameters()):
        assert name1 == name2
        if param1.grad is None or param2.grad is None:
            ...
            np.testing.assert_allclose(
                param1.grad.detach().numpy(), param2.grad.detach().numpy(),
                rtol=1e-7, atol=1e-10)
