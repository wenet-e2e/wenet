...
def test_log_mel_spectrogram(audio_path):
    ...
    log_spec_wenet = next(compute_log_mel_spectrogram(
        [sample], n_fft=N_FFT, hop_length=HOP_LENGTH, num_mel_bins=128, padding=0
    ))["feat"]
    log_spec_wenet = log_spec_wenet.transpose(0, 1).numpy().astype(np.float32)
    log_spec_whisper = whisper.log_mel_spectrogram(audio_path, n_mels=128, padding=0)
...
def test_model(model, audio_path):
    ...
    with torch.no_grad():
        ...
        mel1 = whisper.log_mel_spectrogram(
            audio_path, whisper_model.dims.n_mels, padding=N_SAMPLES).unsqueeze(0)
        ...
        for i, layer in enumerate(whisper_model.encoder.blocks):
            ...
            whisper_layers_ouput.append({"name": "enc.layer{}.attn_ln".format(i),
                                         "value": attn_ln_x.clone()})
            attn_x = layer.attn(attn_ln_x, mask=None, kv_cache=None)[0]
            whisper_layers_ouput.append({"name": "enc.layer{}.attn".format(i),
                                         "value": attn_x.clone()})
            x = x + attn_x
            whisper_layers_ouput.append({"name": "enc.layer{}.attn_residul".format(i),
                                         "value": x.clone()})
            mlp_ln_x = layer.mlp_ln(x)
            whisper_layers_ouput.append({"name": "enc.layer{}.mlp_ln".format(i),
                                         "value": mlp_ln_x.clone()})
            mlp_x = layer.mlp(mlp_ln_x)
            whisper_layers_ouput.append({"name": "enc.layer{}.mlp".format(i),
                                         "value": mlp_x.clone()})
            x = x + mlp_x
            whisper_layers_ouput.append({"name": "enc.layer{}.mlp_residul".format(i),
                                         "value": x.clone()})
        ...
        whisper_decoder_pos = whisper_model.decoder.positional_embedding[
            :whisper_decoder_embed.shape[1], :].unsqueeze(0)  # (B=1, 9, d_model)
        ...
        for i, layer in enumerate(whisper_model.decoder.blocks):
            prev_x = x.clone()
            attn_ln_x = layer.attn_ln(x)
            whisper_layers_ouput.append({"name": "dec.layer{}.attn_ln".format(i),
                                         "value": attn_ln_x.clone()})
            attn_x = layer.attn(attn_ln_x, mask=whisper_model.decoder.mask,
                                kv_cache=None)[0]
            whisper_layers_ouput.append({"name": "dec.layer{}.attn".format(i),
                                         "value": attn_x.clone()})
            x = x + attn_x
            whisper_layers_ouput.append({"name": "dec.layer{}.attn_residul".format(i),
                                         "value": x.clone()})
            cross_attn_ln_x = layer.cross_attn_ln(x)
            whisper_layers_ouput.append({"name": "dec.layer{}.cross_attn_ln".format(i),
                                         "value": cross_attn_ln_x.clone()})
            cross_attn_x = layer.cross_attn(cross_attn_ln_x, whisper_encoder_out,
                                            mask=None, kv_cache=None)[0]
            whisper_layers_ouput.append({"name": "dec.layer{}.cross_attn".format(i),
                                         "value": cross_attn_x.clone()})
            x = x + cross_attn_x
            whisper_layers_ouput.append({"name": f"dec.layer{i}.cross_attn_residul",
                                         "value": x.clone()})
            mlp_ln_x = layer.mlp_ln(x)
            whisper_layers_ouput.append({"name": "dec.layer{}.mlp_ln".format(i),
                                         "value": mlp_ln_x.clone()})
            mlp_x = layer.mlp(mlp_ln_x)
            whisper_layers_ouput.append({"name": "dec.layer{}.mlp".format(i),
                                         "value": mlp_x.clone()})
            x = x + mlp_x
            whisper_layers_ouput.append({"name": "dec.layer{}.mlp_residul".format(i),
                                         "value": x.clone()})
            np.testing.assert_allclose(x.numpy(),
                                       layer(prev_x, whisper_encoder_out,
                                             mask=whisper_model.decoder.mask,
                                             kv_cache=None).numpy(),
                                       rtol=1e-7, atol=1e-10)
        x = whisper_model.decoder.ln(x)
        whisper_logits = (
            x @ torch.transpose(whisper_model.decoder.token_embedding.weight, 0, 1)
        )
        np.testing.assert_allclose(
            whisper_logits.numpy(),
            whisper_model.decoder(whisper_tokens, whisper_encoder_out).numpy(),
            rtol=1e-7, atol=1e-10)

        # 5. Forward wenet.encoder
        waveform, sample_rate = torchaudio.load(audio_path)
        sample = {"wav": waveform, "sample_rate": sample_rate,
                  "key": audio_path, "label": "<N/A>"}
        mel2 = next(compute_log_mel_spectrogram(
            [sample], n_fft=N_FFT, hop_length=HOP_LENGTH,
            num_mel_bins=whisper_model.dims.n_mels, padding=N_SAMPLES
        ))["feat"].unsqueeze(0)
        wenet_mel = pad_or_trim(mel2, N_FRAMES, axis=-2)
        T = wenet_mel.size(1)
        masks = ~make_pad_mask(
            torch.tensor([T], dtype=torch.long), T).unsqueeze(1)  # (B=1, 1, T)
        wenet_embed, pos_emb, masks = wenet_model.encoder.embed(wenet_mel, masks)
        ...
        for i, layer in enumerate(wenet_model.encoder.encoders):
            ...
            wenet_layers_output.append({"name": "enc.layer{}.attn_ln".format(i),
                                        "value": attn_ln_x.clone()})
            x_att, _ = layer.self_attn(
                attn_ln_x, attn_ln_x, attn_ln_x,
                masks, cache=torch.zeros((0, 0, 0, 0)))
            wenet_layers_output.append({"name": "enc.layer{}.attn".format(i),
                                        "value": x_att.clone()})
            x = x + x_att
            wenet_layers_output.append({"name": "enc.layer{}.attn_residul".format(i),
                                        "value": x.clone()})

            mlp_ln_x = layer.norm2(x)
            wenet_layers_output.append({"name": "enc.layer{}.mlp_ln".format(i),
                                        "value": mlp_ln_x.clone()})
            mlp_x = layer.feed_forward(mlp_ln_x)
            wenet_layers_output.append({"name": "enc.layer{}.mlp".format(i),
                                        "value": mlp_x.clone()})
            x = x + mlp_x
            wenet_layers_output.append({"name": "enc.layer{}.mlp_residul".format(i),
                                        "value": x.clone()})
            np.testing.assert_allclose(x.numpy(),
                                       layer(prev_x, masks, pos_emb, masks)[0].numpy(),
                                       rtol=1e-7, atol=1e-10)
        ...
        for i, layer in enumerate(wenet_model.decoder.decoders):
            ...
            attn_ln_x = layer.norm1(x)
            wenet_layers_output.append({"name": "dec.layer{}.attn_ln".format(i),
                                        "value": attn_ln_x.clone()})
            attn_x = layer.self_attn(attn_ln_x, attn_ln_x, attn_ln_x, tgt_mask)[0]
            wenet_layers_output.append({"name": "dec.layer{}.attn".format(i),
                                        "value": attn_x.clone()})
            x = x + attn_x
            wenet_layers_output.append({"name": "dec.layer{}.attn_residul".format(i),
                                        "value": x.clone()})
            assert layer.src_attn is not None
            assert layer.normalize_before
            cross_attn_ln_x = layer.norm2(x)
            wenet_layers_output.append({"name": "dec.layer{}.cross_attn_ln".format(i),
                                        "value": cross_attn_ln_x.clone()})
            cross_attn_x = layer.src_attn(
                cross_attn_ln_x, wenet_encoder_out, wenet_encoder_out, masks)[0]
            wenet_layers_output.append({"name": "dec.layer{}.cross_attn".format(i),
                                        "value": cross_attn_x.clone()})
            x = x + cross_attn_x
            wenet_layers_output.append({"name": f"dec.layer{i}.cross_attn_residul",
                                        "value": x.clone()})
            assert layer.normalize_before
            mlp_ln_x = layer.norm3(x)
            wenet_layers_output.append({"name": "dec.layer{}.mlp_ln".format(i),
                                        "value": mlp_ln_x.clone()})
            mlp_x = layer.feed_forward(mlp_ln_x)
            wenet_layers_output.append({"name": "dec.layer{}.mlp".format(i),
                                        "value": mlp_x.clone()})
            x = x + mlp_x
            wenet_layers_output.append({"name": "dec.layer{}.mlp_residul".format(i),
                                        "value": x.clone()})
            np.testing.assert_allclose(
                x.numpy(), layer(prev_x, tgt_mask, wenet_encoder_out, masks)[0].numpy(),
                rtol=1e-7, atol=1e-10)
    ...
    np.testing.assert_allclose(whisper_mel.numpy(), wenet_mel.transpose(1, 2).numpy(),
                               rtol=1e-7, atol=1e-10)
    np.testing.assert_allclose(whisper_model.encoder.positional_embedding.numpy(),
                               pos_emb.squeeze(0).numpy(),
                               rtol=1e-7, atol=1e-10)
    np.testing.assert_allclose(whisper_subed.numpy(), wenet_subed.numpy(),
                               rtol=1e-7, atol=3e-7)
    np.testing.assert_allclose(whisper_embed.numpy(), wenet_embed.numpy(),
                               rtol=1e-7, atol=1e-10)
