  ...
  for (int32 f = 0; f <= num_frames; f++) {
    for (Token *tok = active_toks_[f].toks; tok != NULL; tok = tok->next) {
      ...
      for (ForwardLinkT *l = tok->links; l != NULL; l = l->next) {
        ...
        StateId tmp = l->is_end_boundary ? ofst->AddState() : nextstate;
    ...
    if (tok->tot_cost <= cur_cutoff) {
      for (fst::ArcIterator<FST> aiter(*fst_, state); !aiter.Done();
        ...
        if (arc.ilabel != 0) {  // propagate..
          ...
          if (context_graph_) {
            if (arc.olabel == 0) {
              e_next->val->context_state.sid = tok->context_state.sid;
            } else {
              e_next->val->context_state =
                  context_graph_->GetNextState(tok->context_state, arc.olabel);
              is_start_boundary = e_next->val->context_state.is_start_boundary;
              is_end_boundary = e_next->val->context_state.is_end_boundary;
              graph_cost -= e_next->val->context_state.score;
            }
          }
    ...
    for (fst::ArcIterator<FST> aiter(*fst_, state); !aiter.Done();
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        ...
        if (tot_cost < cutoff) {
          ...
          if (context_graph_) {
            if (arc.olabel == 0) {
              e_new->val->context_state.sid = tok->context_state.sid;
            } else {
              e_new->val->context_state =
                  context_graph_->GetNextState(tok->context_state, arc.olabel);
              is_start_boundary = e_new->val->context_state.is_start_boundary;
              is_end_boundary = e_new->val->context_state.is_end_boundary;
              graph_cost -= e_new->val->context_state.score;
            }
          }

          tok->links = new ForwardLinkT(e_new->val, 0, arc.olabel, graph_cost,
                                        0, is_start_boundary, is_end_boundary,
                                        tok->links);
