...
class LatticeDeterminizerPruned {
 public:
  ...
  void Output(MutableFst<CompactArc>* ofst, bool destroy = true) {
    ...
    for (StateId this_state_id = 0; this_state_id < nStates; this_state_id++) {
      ...
      // memory, and we want to reduce the maximum amount ever allocated.
      if (destroy) {
        vector<TempArc> temp;
        temp.swap(this_vec);
      }
  ...
  bool Determinize(double* effective_beam) {
    ...
    while (!queue_.empty()) {
      ...
      if ((opts_.max_states > 0 && num_states > opts_.max_states) ||
        ...
        // example where memory was exhausted before we reached state #100.
        ...
        // is what we'll return...  because we expanded stuff in order of total
 ...
 private:
  ...
  typedef unordered_map<const vector<Element>*, Element, SubsetKey, SubsetEqual>
      InitialSubsetHash;
  ...
  OutputStateId MinimalToStateId(const vector<Element>& subset,
    ...
    // queue at this point.  Here, the queue happens elsewhere, and we directly
    // process the state (which result in stuff getting added to the queue).
  ...
  // Elements represents a set of next-states with associated weights and
  // strings, each one arising from an arc from some state in a
  // determinized-state; the next-states are unique (there is only one Entry
  // assocated with each)
  ...
  void InitializeDeterminization() {
    ...
    if (start_id != kNoStateId) {
      /* Create determinized-state corresponding to the start state....
         ...
         is actually what we want, as otherwise we'd have problems dealing with
         any extra weight and string and might have to create a "super-initial"
         state which would make the output nondeterministic.  Normalization is
         only needed to make the determinized output more minimal anyway, it's
         not needed for correctness. Note, we don't put anything in the
         initial_hash_.  The initial_hash_ is only a lookaside buffer anyway, so
         this isn't a problem-- it will get populated later if it needs to be.
      */
  ...
  struct OutputState {
    ...
    // We always process the final-weight regardless of the beam; when producing
    // the output we may have to ignore some of these.
    ...
    // OutputState; this is because of the priority-queue we use, that ensures
    // that the "best" path into the state will be expanded first.
  ...
  // guaranteed to have "take-the-minimum" semantics).  We get the double from
  // the ConvertToCost() function on the lattice weights.
...
//       Arc arc = aiter.Value();
//
//       // Note: the words are on the input symbol side and transition-id's are
//       on
...
//
// /** This function does a first pass determinization with phone symbols
// inserted
//     at phone boundary. It uses a transition model to work out the
//     transition-id to phone map. First, phones will be inserted into the word
//     level lattice. Second, determinization will be applied on top of the
//     phone + word lattice. Finally, the inserted phones will be removed,
//     converting the lattice back to a word level lattice. The output lattice
//     of this pass is not deterministic, since we remove the phone symbols as a
//     last step. It is supposed to be followed by another pass of
//     determinization at the word level. It could also be useful for some other
//     applications such as fMLLR estimation, confidence estimation,
//     discriminative training, etc.
// */
// template<class Weight, class IntType>
...
//   bool ans = true;
//
//   // Make sure at least one of opts.phone_determinize and
//   opts.word_determinize
...
//
//     // If --word-determinize is false, we've finished the job and return
//     here. if (!opts.word_determinize) {
//       // We are expecting the words on the input side.
//       ConvertLattice<Weight, IntType>(*ifst, ofst, false);
//       return ans;
//     }
...
//     KALDI_VLOG(3) << "Doing second pass of determinization on word
//     lattices."; ans = DeterminizeLatticePruned<Weight, IntType>(
...
//       KALDI_ERR << "Topological sorting of state-level lattice failed
//       (probably"
//                 << " your lexicon has empty words or your LM has epsilon
//                 cycles"
//                 << ").";
//     }
//   }
