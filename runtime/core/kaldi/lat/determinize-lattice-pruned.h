...
struct DeterminizeLatticePhonePrunedOptions {
  ...
  void Register(kaldi::OptionsItf* opts) {
    ...
    opts->Register("word-determinize", &word_determinize,
                   "If true, do a second "
                   "pass of determinization on words only (see also "
                   "--phone-determinize)");
    opts->Register("minimize", &minimize,
...
//     when we insert phones into the lattice, we map the original phone label
//     to (first_phone_label + original_phone_label). It is supposed to be used
...
//     MutableFst<ArcTpl<Weight> > *fst);
//
// /** This function is a wrapper of DeterminizeLatticePhonePrunedFirstPass()
// and
//     DeterminizeLatticePruned(). If --phone-determinize is set to true, it
//     first calls DeterminizeLatticePhonePrunedFirstPass() to do the initial
//     pass of determinization on the phone + word lattices. If
//     --word-determinize is set true, it then does a second pass of
//     determinization on the word lattices by calling
//     DeterminizeLatticePruned(). If both are set to false, then it gives a
//     warning and copying the lattices without determinization.
//
//     Note: the point of doing first a phone-level determinization pass and
//     then a word-level determinization pass is that it allows us to
//     determinize deeper lattices without "failing early" and returning a
//     too-small lattice due to the max-mem constraint.  The result should be
//     the same as word-level determinization in general, but for deeper
//     lattices it is a bit faster, despite the fact that we now have two passes
//     of determinization by default.
// */
// template<class Weight, class IntType>
...
//       = DeterminizeLatticePhonePrunedOptions());
//
// /** This function is a wrapper of DeterminizeLatticePhonePruned() that works
// for
...
//     requires "ifst" to have transition-id's on the input side and words on
//     the output side. This function can be used as the top-level interface to
//     all the determinization code.
// */
