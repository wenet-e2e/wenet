...
TEST(FeaturePipelineTest, BlockingQueueTest) {
  ...
  ASSERT_EQ(capacity_queue->Size(), 2);
  auto pop_data = capacity_queue->Pop(3);  // 1,2,3 num > capacity
  ASSERT_EQ(pop_data.size(), 3);
  ASSERT_EQ(pop_data[2], 3);
  std::this_thread::sleep_for(std::chrono::milliseconds(5));
  ASSERT_EQ(capacity_queue->Size(), 2);  // capacity_queue:4,5
  ASSERT_EQ(capacity_queue->Pop(), 4);   // capacity_queue:5
  ASSERT_EQ(capacity_queue->Size(), 1);  // capacity_queue:5
  push_thread.join();                    // capacity_queue:5
