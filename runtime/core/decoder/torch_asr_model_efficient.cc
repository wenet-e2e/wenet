...
#ifndef IOS
void TorchAsrModelEfficient::InitEngineThreads(int num_threads) {
...
void TorchAsrModelEfficient::Read(const std::string& model_path) {
...
std::shared_ptr<AsrModel> TorchAsrModelEfficient::Copy() const {
  auto asr_model = std::make_shared<TorchAsrModelEfficient>(*this);
  // Reset the inner states for new decoding
  asr_model->Reset();
  return asr_model;
}

void TorchAsrModelEfficient::Reset() {
  offset_ = 0;
...
float TorchAsrModelEfficient::ComputeAttentionScore(const torch::Tensor& prob,
...
void TorchAsrModelEfficient::AttentionRescoring(
