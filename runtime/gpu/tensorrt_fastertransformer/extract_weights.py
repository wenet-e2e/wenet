...
def export_GetAllWeight(model, gsg):
    ...
    cur_idx = 0
    for w in model.graph.initializer:
        if w.name in not_name and len(w.dims) ==2 and (w.dims[0] == 256
         or w.dims[0] == 512) and (w.dims[1] == 256 or w.dims[1] == 512):
            for node in gsg.nodes:
                if node.op == "MatMul" and node.i(0,0).op == "Slice" and node.inputs[1].name == w.name:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, required=True, help="output weights dir")
    ...
    for name in result:
        ...
        cur = result[name]
        if name.endswith(".weight") and len(cur.shape)==2 and "decoder.embed.0.weight" not in name:
